...
public class Matchers {
    ...
    /**
     * Get a matcher that checks that a {@link Generator} eventually generates all the expected values and not any
     * unexpected ones.
     ...
     * @return the matcher.
     */
    public static <T> TypeSafeDiagnosingMatcher<Generator<T>> eventuallyGeneratesAllValues(
            final Set<T> values, final int attempts) {
        return new TypeSafeDiagnosingMatcher<Generator<T>>() {

            @Override
            protected boolean matchesSafely(Generator<T> generator, Description mismatchDescription) {
                ...
                for(int i = 0; i < attempts && !assessment.matchedSuccessfully(); i++){
                    T generatedValue = generator.generate();
     ...
     * @return the matcher.
     */
    public static <T extends Number,U extends Number> TypeSafeDiagnosingMatcher<Distribution<T,U>> eventuallyGeneratesAllValues(
    ...
    /**
     * Get a matcher that looks at a derivative of the values generated by a generator and checks that a set of the
     * possible derivatives is eventually matched. A bit abstruse but to take a concrete example - we expect a Generator
     ...
     * @param derivativeValues The set of derived values that we expect to get from the generator.
     * @param deriver An object that derives a value from the generated value that can usefully be asserted against.
     * @param attempts The maximum number of times that values can be generated and derived, attempting to match the
     *                 expected values.
     * @param <T> The type of value being generated.
     * @param <U> The type of the value being derived.
     * @return the matcher.
     */
    public static <U, T> TypeSafeDiagnosingMatcher<Generator<T>> eventuallyGeneratesAllDerivatives(
